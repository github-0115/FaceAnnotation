...
func StopTask(c *gin.Context) {
	...
	for _, smallTask := range smallTasks {
		if smallTask.Status != smalltaskmodel.TaskStatus.Success {
			err := smalltaskmodel.UpdateSmallTasks(smallTask.SmallTaskId, smalltaskmodel.TaskStatus.Stop)
			if err != nil {
				log.Error(fmt.Sprintf("stop  small tasks  update status err %s", err))
			}
		}
	}

	err = taskmodel.UpdateTaskStatus(taskColl.TaskId, taskmodel.TaskStatus.Stop)
	if err != nil {
